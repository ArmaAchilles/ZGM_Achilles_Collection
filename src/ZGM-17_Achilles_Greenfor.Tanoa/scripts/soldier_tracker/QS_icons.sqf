...
Version:
	...
	2.3.0 (released 3/10/2016 A3 v1.64)
...
Last Modified: 
	...
	3/10/2016 A3 1.64 by Quiksilver
...
#define UNIT_MARKER_ZOOM_CLOSE_UPPERLIMIT 	0.05	// player name in larger font
#define UNIT_MARKER_ZOOM_FAR_LOWERLIMIT 	0.05	// player name in smaller font
#define UNIT_MARKER_ZOOM_FAR_UPPERLIMIT 	0.3		// player name in smaller font
...
_QS_fnc_isIncapacitated = {
	...
	if (_medicalSystem isEqualTo 'BIS') then {
		if ((lifeState _u) isEqualTo 'UNCONSCIOUS') then {
			_cond = TRUE;
		};
	} else {
...
_QS_fnc_iconText = {
	...
	if (((_v distance player) < (_QS_ST_X select 68)) || {(_isAdmin)}) then {
		if ((_ms < UNIT_MARKER_ZOOM_FAR_UPPERLIMIT) || {(_isAdmin)}) then {
			if ((_ms > UNIT_MARKER_ZOOM_FAR_LOWERLIMIT) || {(_isAdmin)}) then {
				if (_showMOS) then {
				...
				if (_ms <= UNIT_MARKER_ZOOM_CLOSE_UPPERLIMIT) then {
					if (_showMOS) then {
						_t = format ['%1 [%2]',_vn,_vt];
					} else {
						_t = format ['%1',_vn];
		...
		if (_ms < UNIT_MARKER_ZOOM_FAR_UPPERLIMIT) then {
			if (_ms > UNIT_MARKER_ZOOM_FAR_LOWERLIMIT) then {
				_t = format ['%1',_vn];
			} else {
				if (_ms <= UNIT_MARKER_ZOOM_CLOSE_UPPERLIMIT) then {
					_t = format ['%1',_vn];
