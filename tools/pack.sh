...
function Achilles_Bash_fnc_genMission {
	...
	AddonBuilder "$(cygpath -w "$PWD/tmp/$folder_name")" "$(cygpath -w "$PWD/pbo")" 1>/dev/null 2>&1
}
export -f Achilles_Bash_fnc_genMission

# pack the template mission
AddonBuilder "$(cygpath -w "$PWD/src/$template_folder")" "$(cygpath -w "$PWD/pbo")" 1>/dev/null 2>&1
